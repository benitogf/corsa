...
else{ global.tones = code(); }}(this, function(){
    var tones = {
        ...
        playFrequency: function(freq) {
            ...
            //this.envelope.gain.setValueAtTime(this.volume, this.context.currentTime);
            ...
            if(this.release) {
                ...
                setTimeout(function() {
                 if (tones.oscs.length > 0) {
                    //var osc = tones.oscs.pop();
                    // var envelope = tones.gains.pop();
                    osc.stop();
                    ...
                    envelope.disconnect( this.analyser );
            ...
            if (this.type === 'custom') {
               var real = new Float32Array(this.waveRange.length);
               var imag = new Float32Array(this.waveRange.length);
               this.waveRange.forEach(function(wr, i){
                 real[i] = tones.wave(wr);
                 imag[i] = wr;
                 //console.log(real[i]);
               });
              //  console.log(real);
